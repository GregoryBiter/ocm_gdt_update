<?php
// Heading
$_['heading_title']          = 'GDT Updater';

// Text
$_['text_extension']         = 'Extensions';
$_['text_success']           = 'Success: You have modified GDT Updater module!';
$_['text_edit']              = 'Edit GDT Updater Module';
$_['text_check_updates']     = 'Check for Updates';
$_['text_updates_found']     = 'Found %s updates available. Check the list below.';
$_['text_no_updates']        = 'All modules are up to date.';
$_['text_update_success']    = 'Module %s has been successfully updated.';
$_['text_current_version']   = 'Current Version';
$_['text_new_version']       = 'New Version';
$_['text_update']            = 'Update';
$_['text_latest']            = 'Latest Version';
$_['text_up_to_date']        = 'Up to Date';
$_['text_installed_modules'] = 'Installed Modules';
$_['text_backups']           = 'Резервні копії';
$_['text_backup_date']       = 'Дата створення';
$_['text_from_version']      = 'З версії';
$_['text_to_version']        = 'На версію';
$_['text_restore_success']   = 'Модуль %s успішно відновлено до версії %s.';
$_['text_no_backups']        = 'Резервні копії для цього модуля не знайдено.';
$_['text_confirm_restore']   = 'Ви впевнені, що хочете відновити цей модуль до версії %s? Це замінить всі поточні файли модуля з резервної копії.';

// Column
$_['column_name']            = 'Module Name';
$_['column_description']     = 'Description';
$_['column_action']          = 'Action';

// Entry
$_['entry_server']           = 'URL сервера оновлень';
$_['entry_client_id']        = 'ID клієнта';
$_['entry_api_key']          = 'API-ключ';
$_['entry_status']           = 'Статус';

// Help
$_['help_client_id']         = 'Введіть ID клієнта, наданий адміністратором сервера оновлень.';
$_['help_api_key']           = 'Введіть API-ключ, наданий адміністратором сервера оновлень. Це необхідно для аутентифікації на сервері.';

// Button
$_['button_check']           = 'Check Now';
$_['button_update']          = 'Update';
$_['button_restore']         = 'Відновити';
$_['button_backups']         = 'Резервні копії';
$_['button_back']            = 'Назад';

// Error
$_['error_permission']       = 'Warning: You do not have permission to modify GDT Updater module!';
$_['error_server']           = 'Update server URL is not configured.';
$_['error_server_empty']     = 'Update server URL cannot be empty.';
$_['error_no_modules']       = 'No GDT modules found in your system.';
$_['error_code']             = 'Module code not specified.';
$_['error_module_not_found'] = 'Module not found.';
$_['error_no_update']        = 'No update available for this module.';
$_['error_download']         = 'Failed to download the update package.';
$_['error_zip']              = 'Failed to open the update package.';
$_['error_curl']             = 'Помилка cURL: %s';
$_['error_http']             = 'Код HTTP помилки: %s';
$_['error_auth']             = 'Помилка аутентифікації. Будь ласка, перевірте ID клієнта та API-ключ.';
$_['error_server_auth']      = 'Помилка аутентифікації на сервері оновлень: %s';
$_['error_backup_not_found'] = 'Резервну копію не знайдено.';
$_['error_backup_info']      = 'Файл інформації про резервну копію не знайдено.';
$_['error_backup_info_parse'] = 'Не вдалося прочитати інформацію про резервну копію.';
$_['error_backup_files']     = 'Файли резервної копії не знайдено.';
$_['error_restore']          = 'Не вдалося відновити модуль: %s';
